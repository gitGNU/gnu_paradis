...
public class UDPPeer
{
	...
	try
	{
	    ...
	    connect(new StreamDatagramSocket(pubip, PORT), monitor, peers);
	...
	catch (final Throwable err)
	{
	    ...
	    StreamDatagramSocket _server = null;
	
	    if (serverauth)
		try
		{
		    System.out.println("Trying to make local server");
		    _server = new StreamDatagramSocket(PORT);
		}
		...
		_server = null;
            ...
            final StreamDatagramSocket server = _server;
	...
	for (final String remote : remotes)
		...
		connect(new StreamDatagramSocket(remote, PORT), monitor, peers);
    ...
    private static void connect(final StreamDatagramSocket sock, final Object monitor, final String[] peers) throws IOException
    {
